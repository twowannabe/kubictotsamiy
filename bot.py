...
def generate_answer_by_topic(user_question, related_messages, max_chars=1000):
    ...
    # Формируем prompt для создания "мнения автора" на основе сообщений
    prompt = "На основе приведенных ниже сообщений пользователя, сформулируйте связное мнение от его имени. Используйте только эти сообщения для формирования ответа и не добавляйте дополнительной информации.\n\n"
    prompt += "Сообщения пользователя:\n"
    prompt += "\n".join(truncated_messages)
    prompt += f"\n\nВопрос пользователя: {user_question}\nСвязное мнение автора, основанное на его сообщениях:"

    try:
        response = openai.ChatCompletion.create(
            model='gpt-4o-mini',  # Используем gpt-4o-mini или другую поддерживаемую модель
            messages=[
                {"role": "system", "content": "Вы помощник, который формирует связное мнение на основе сообщений пользователя, не добавляя ничего нового."},
